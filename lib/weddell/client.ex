defmodule Weddell.Client do
  ...
  @default_endpoint Application.get_env(:weddell_medici, :rpc_base_url)
  ...
  def init(:ok) do
    connect(
      Application.get_env(:weddell_medici, :project),
      Application.get_all_env(:weddell_medici)
    )
  end
  ...
  def connect(project, opts \\ []) do
    IO.puts("endpoint: #{@default_endpoint}")
    scheme = Keyword.get(opts, :scheme, :https)
    ssl = ssl_opts(opts)
    cred = if scheme == :https, do: GRPC.Credential.new(ssl: ssl), else: nil
    endpoint = Keyword.get(opts, :host, @default_endpoint)

    {:ok, channel} =
      Stub.connect("#{endpoint}",
        cred: cred,
        adapter_opts: %{
